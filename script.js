        ...
        input.addEventListener("keypress", (e) => {
          if (e.key === "Enter") {
            textSpan.textContent =
              input.value.trim() || textSpan.textContent;
...
function updateTaskCount() {
  let totalTask =
    document.getElementById("task-container").getElementsByTagName("div")
      .length;
  let completedTask = document.querySelectorAll(
  ...
  let progressLength =
    totalTask > 0 ? (completedTask / totalTask) * 100 : 0;
  progress.style.width = `${progressLength}%`;

  if (completedTask === totalTask && totalTask > 0 && !confetti) {
